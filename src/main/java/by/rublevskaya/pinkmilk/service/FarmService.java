...
public class FarmService {
    ...
    public void addCow(Cow cow) {
        ...
        if (cow.getMilkPerDay() < 0) {
            CustomLogger.warning("Отрицательное количество молока для коровы: " + cow);
            throw new IllegalArgumentException("Количество молока не может быть отрицательным.");
        }
        cows.add(cow);
    ...
    public void addMilkToWarehouse(Milk milk) {
        ...
        Product milkProduct = new Product("Молоко", milk.getVolume(), 3); // Срок хранения 3 дня
    ...
    public void saveState(String filePath) {
        if (filePath == null || filePath.isEmpty()) {
            ...
            throw new IllegalArgumentException("Путь сохранения не может быть null или пустым.");
        }
        try {
            objectMapper.writeValue(new File(filePath), cows);
